require "acts_as_tree_on_steroids/version"

module ActsAsTreeOnSteroids
  ...
  def ancestors(reload=false, include_node = false)
          return nil if is_root? || self.id_path.blank? || self.id_path.first == ','
  ...
  # saves the record disabling all validation and callbacks that are triggered for tree calculation.
        ...
        def after_create #:nodoc:
          #logger.debug "aatos aftercreate"
        ...
        def before_update #:nodoc:
          #logger.debug "aatos beforeupdate start"
          unless @skip_callbacks
            calculate_id_path
            calculate_fields
            begin
              update_qualified_name
            rescue
            end
            #logger.debug "qn #{qualified_name}"
          end
          #logger.debug "aatos beforeupdate end"
        end

        def after_update #:nodoc:
          #logger.debug "aatos afterupdate"
          unless @skip_callbacks
            propagate_changes
          end
        end
        ...
        def calculate_fields
          #logger.debug "aatos calculate_fields start"
          #id_path
          if self.parent.nil?
          ...
          #logger.debug "aatos calculate_fields end"
        end
        ...
        def calculate_id_path
          #logger.debug "aatos calculate id path start"
          ...
          #logger.debug "aatos calculate id path end"
        end

        def propagate_changes
          #logger.error "AATS: pc for #{self.id} #{self.qualified_name}"
          #update parent's children count
          if self.has_changed_parent?
            #logger.error "AATS: changed parent"
            ...
            unless parent_id_was.nil?
              begin
                #logger.error "AATS: changed parent recalc2"
          ...
          if !self.is_leaf? && self.changed? && !(self.children_count_changed? && self.changed == ['children_count'])
            for child in self.children
              #logger.error "AATS: child recalc2 for #{self.id} #{self.qualified_name} and changed #{self.changed.join(',')}"
              child.recalc
            end
          end
        end
